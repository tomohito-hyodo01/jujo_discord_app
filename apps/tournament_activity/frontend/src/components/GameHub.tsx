...
export default function GameHub({ username, discordId, onExitToPortal }: GameHubProps) {
  const [selected, setSelected] = useState<null | 'rpg' | 'run' | 'run-exer' | 'react'>(null)
  ...
  return (
    <div style={{ padding: '28px 20px', maxWidth: 820, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
        ...
        {isAdmin && card(() => setSelected('run-exer'), '🏃', 'エビ走（運動）', 'スマホを持って実際にジャンプ！体を動かして遊ぶエクサゲーム版（管理者向け試作）。運動になる！ランキングは通常モードとは別。', '#14b8a6')}
